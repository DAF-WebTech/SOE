...
results.data.forEach(function (record) {
	if (!bioregions[record.Bioregion]) {
		bioregions[record.Bioregion] = {}
	}
	bioregions[record.Bioregion][record.Group] = record;
...
Object.keys(bioregions).forEach(function (bioregionName) {
	...
	if (bioregionName == "Queensland") {
		var heading = String.format("Area of Queensland pre-clear threatened flora habitat and {0} remnant habitat by species group", latestYear)
	}
	else {
		var heading = String.format("Area of pre-clear threatened flora habitat and {0} remnant habitat by species group in the {1}", latestYear, bioregionName)
	}

	var arrayTable = [["Flora Group", "Pre-clear", "Remnant"]]
	...
	if (bioregionName == "Queensland") {
		heading = String.format("Proportion of Queensland pre-clear threatened flora habitat that is remnant and non-remnant habitat, {0}", latestYear)
	}
	else {
		heading = String.format("Proportion of pre-clear threatened flora habitat that is remnant and non-remnant habitat in the {0}, {1}", bioregionName, latestYear)
	}

	arrayTable = [["Flora Group", "Remnant", "Non-remnant"]]
	groups.forEach(function (group) {
		var record = bioregion[group]
